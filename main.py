    ...
    if os.path.exists(file_path):
        ...
        if lines:
            for path in lines:
                ...
                ctk.CTkButton(frame, text="⨉", fg_color="#e17055", width=30, command=lambda p=path: update_history(p, add=False)).pack(side="right", padx=(2, 5))
                ctk.CTkButton(frame, text="↻", fg_color="#0984e3", width=30, command=lambda p=path: reorganize_folder(p)).pack(side="right", padx=(5, 2))
        else:
